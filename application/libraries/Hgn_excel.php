...
class Hgn_excel {
	...
	public function create($filename, $data=array()){
		...
		// log_message('error', json_encode($this->data['title']));

		foreach ($this->data['body'] as $key => $value) {
			$i = 0;
			foreach ($this->data['title'] as $key2 => $value2) {
				// log_message('error', ''.$this->_getCellHuruf($key2).$this->_getCellAngka($key).' => '. $value[$value2['id']]);
				$isi = $value[$value2['id']];
	...
	private function _createHeader(){
		...
		$this->phpexcel->getProperties()->setCreator('PRIME APPS')
									 ->setLastModifiedBy('')
		...
		// Rename worksheet
		$this->phpexcel->getActiveSheet()->setTitle($this->header['sheet_name']);
	...
	private function _getCellHuruf($no=0){
		...
		$result = '';
		if($no > 25){
			$result = $alpha[floor($no/26)].$alpha[$no%26];
		}else{
			$result = $alpha[$no%26];
		}
