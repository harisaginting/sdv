  ...
  <div class="card-body">
   <form method="POST" enctype="multipart/form-data" id="frmAdd">
    <div class="row">
        ...
        <div class="col-sm-6">
            ...
            <div class="form-group">
              ...
              <select id="regional" name="regional" class="form-control form-control-sm"  style="width: 100%;">
                    <option></option>
                    <option 
            ...
            <div class="form-group">
              ...
              <select id="type" name="type" class="form-control form-control-sm Jselect2"  style="width: 100%;">
                    ...
                    <option value="CPE DEVICES"
                    <?= !empty($data['TYPE'])&&($data['TYPE']=='CPE DEVICES')  ? 'selected' : ''; ?>>CPE DEVICES</option>
            ...
            <div class="form-group">
              <label>No. Kontrak Bersama (KB)</label>
              <input type="text" name="no_kb" id="no_kb" class="form-control" placeholder="insert No. KB" value="<?= !empty($data['NO_KB']) ? $data['NO_KB'] : ''; ?>">
            </div>

            <div class="form-group">
              <label>No. Kontrak Layanan</label>
                <div class="form-group" id="c_no_kl">
                    
                </div>
                <div class="input-group">
                  <input class="form-control" id="add_no_kl" type="text" name="add_no_kl" placeholder="Insert No. Kontrak Layanan">
                  <span class="input-group-append">
                  <button id="add_more_kl" class="btn btn-primary" type="button"><i class="fa fa-plus"></i></button>
                  </span>
                </div>
            </div>

            <div class="boxes">
                    <input type="checkbox" id="manage_service" name="manage_service"  data-val="manage_service" <?= (!empty($data['MANAGE_SERVICE'])&&($data['MANAGE_SERVICE']==1)) ? 'checked' : ''; ?> >
                  ...
                  <div class="col-md-3">
                    <div class="form-group">
                      <label class=" control-label">Document BAKN/P8</label>
                        ...
                        <input id="doc_bakn" name="doc_bakn" type="file" class="form-control file" >
                    </div>
                  </div>
                  <div class="col-md-3">
                    <div class="form-group">
    ...
    <div class="row m-top-30">
        <?php if($data['STATUS']=='LEAD' || $data['STATUS']=='LAG' || $data['STATUS']=='DELAY') : ?>
        ...
        <div class="col-sm-2">
            <button id="btn_addendumProject" type="button" style="width: 100%;" class="btn btn-success btn-addon"><i class="fa fa-floppy-o"></i>
             &nbsp; Addendum End Date
            </button>        
        </div>
...
<div class="modal fade"  role="dialog" aria-labelledby="modalCloseDocument" aria-hidden="true" id="modalCloseDocument">
  <div class="modal-dialog modal-lg modal-primary">
    <div class="modal-content">
      ...
      <h4 class="modal-title" id="modal-title-deliverable">Project Close</h4>
        ...
        <div class="modal-header">
              ...
              <div class="modal-body relative">
                        <form method="POST" enctype="multipart/form-data" id="frmPartner">
                            <div class="form-group">
                              <select id="m-partner" name="m-partner" class="form-control form-control-sm">
                                    <option></option>
                            ...
                            <div class="form-group">
                              <select id="m-payment" name="m-payment" class="form-control form-control-sm">
                                    <option></option>
  ...
  var Page = function () {   
      ...
      <?php if(!empty($partners)) : foreach($partners as $key => $value): ?>
          table.row.add( [
                      ...
                      <?php if(!empty($partners[$key]['LINK_P82'])): ?>
                         "<span><a href='<?= $partners[$key]['LINK_P82']; ?>' target='_blank'>Link SPK</a><span>"+
                         "<input name='document_spk[]' type='file' class='form-group' id='document_spk"+counter+"' data-show-preview='false' />",
                      <?php else : ?>
                          "<input name='document_spk[]' type='file' class='form-group' id='document_spk"+counter+"' data-show-preview='false' />",
                      <?php endif;?>
      ...
      return {
          init: function() { 
              ...
              $(document).on('click','#add_more_kl', function (e) {
                e.stopImmediatePropagation();
                  ...
                  span.setAttribute('id','span'+co);

                  var button = document.createElement("button");
                  button.setAttribute('class','btn btn-danger btnDelKL');
                  button.setAttribute('type','button');
                  button.setAttribute('data-id',co);
                  button.setAttribute('id','btn'+co);

                  var i      = document.createElement('i');
                  i.setAttribute('class','fa fa-trash');

                  var input = document.createElement("input");  
                  input.setAttribute('name', 'no_kl[]');
                  input.setAttribute('id', 'id_tmp_kl'+co);
                  ...
                  $("#div"+co).append(input); 
                  $("#div"+co).append(span); 
                  $("#span"+co).append(button); 
                  $("#btn"+co).append(i); 

                  $('#id_tmp_kl'+co).val($('#add_no_kl').val());
                  $('#div'+co).css('margin-top',5);
                  $('#add_no_kl').val('');
                  co++;
              });
              ...
              <?php 
              if(!empty(($data['NO_KL']) && ($data['NO_KL']!= 'null'))) :  
                $no_kl = json_decode($data['NO_KL']);
                if(!empty($no_kl)) :
                foreach ($no_kl as $key => $value) :
              ?>
                var div   = document.createElement("div");
                  div.setAttribute('class','input-group');
                  div.setAttribute('id','div'+co);
              ...
              $(document).on('click','.btnDelKL', function (e) {
                e.stopImmediatePropagation();
                e.preventDefault();
                var id = $(this).data('id');
                console.log(id);
