...
<div class="col-sm-12">
  ...
  <div class="pull-right btn btn-xs btn-danger  circle2" data-id= "<?= $id_project ?>"  id="btn-delete_candidate" style="margin-left: 10px;"><i class="fa fa-trash"></i></div>
   ...
   <form method="POST" enctype="multipart/form-data" id="frmAdd">
    <div class="row"> 
        <div class="col-sm-6 merah">
            ...
            <?php if(!empty($quote_so[0]['NO_SO'])&&!empty($data['ID_LOP_EPIC'])) : ?>
             <div class="form-group">
              <label>No. Quote and SO </label>
                      <table class="table" style="border: 2px #20a8d8 solid;">
                        <thead>
                          <th>NO QUOTE</th>
                          <th>NO SO</th>
                        </thead>
                        <tbody>
                          <?php foreach ($quote_so as $key => $value) {
                            echo '<tr><td>'.$value['NO_SO'].'</td><td>'.$value['NO_SO'].'</td></tr>'; 
                          } ?> 
                        </tbody>
                      </table>   
            </div>
        ...
        <div class="col-sm-6">
            <div class="form-group">
              <label for="name">No SPK Customer</label>
              <input type="text" class="form-control"  
              value="<?= !empty($data['NO_SPK_CC'])? $data['NO_SPK_CC'] : '' ?>" required readOnly>
            </div>  
      ...
      return {
          init: function() { 
              ...
              $(document).on('click','#assignNonPM', function (e) {
               ...
               $(document).on('click','#btn-delete_candidate', function (e) {
                e.stopImmediatePropagation();
                e.preventDefault();
                var id = $(this).data('id');
                console.log(id);
                          ...
                          if(result){
                              $.ajax({
                                            ...
                                            $('#pre-load-background').fadeOut();
                                             if(result.data=='success'){
                                              bootbox.alert("Success!", function(){ 
                                              window.location.href = base_url+'projects/view_candidate/<?= $id_project; ?>' ;
                                              });
                                            }else{
                                              bootbox.alert("Failed!", function(){});
                                              }
