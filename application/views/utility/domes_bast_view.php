...
<li class="breadcrumb-item active nav-link-hgn" data-url="<?= base_url(); ?>utility/domes_bast_view/<?= $id_bast; ?>"> <strong>view edit</strong></li>
</ol>
...
<div class="col-sm-12">
  ...
  <div class="card-body">
    <form id="formBast">
      ...
      <?php if(empty($bast['NIPNAS'])) : ?>
        <div class="col-sm-12">
          <div class="card text-white bg-warning">
            <div class="card-body">
              <div class="text-value h3">* NIPNAS for customer has empty, please select customer from avaliable list</div>
            </div>
          </div>
        </div>
      <?php endif; ?>
      ...
      <div class="col-sm-6">
          ...
          <div class="form-group">
            ...
            <select style="width: 100%;" name="segmen" id="segmen" class="form-control Jselect2">
                ...
                <?php if(!empty($bast['SEGMENT'])) { ?>
                <option  value= "<?= $bast['SEGMENT'];?>" selected><?= $bast['SEGMENT']; ?></option>   
                <?php } else { ?>
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control rupiah" id="value" name="value" placeholder="Project Value" required value="<?= !empty($bast['NILAI_PEKERJAAN']) ? $bast['NILAI_PEKERJAAN'] : '';   ?>">
          </div>
          ...
          <div class="form-group ">
            ...
            <div class="input-group">
              <select name="status" id="status" class="form-control text-primary" style="height: inherit;">
                    ...
                    <option value="<?= $bast['STATUS']; ?>" style="color:#000 !important;"><?= $bast['STATUS']; ?></option>
                    <option value="CHECK BY SE DI" <?= (!empty($bast['STATUS'])&&$bast['STATUS']=='CHECK BY SE DI') ? ' selected' : '' ?> >CHECK BY SE PMO</option>
                    <option value="APPROVED" <?= (!empty($bast['STATUS'])&&$bast['STATUS']=='APPROVED') ? ' selected' : '' ?> >APPROVED</option>  
                    <option value="REVISION" <?= (!empty($bast['STATUS'])&&$bast['STATUS']=='REVISION') ? ' selected' : '' ?> >REVISION</option>
                    <?php endif; ?> 
    ...
    <div class="row m-top-3">
        <div class="col-sm-2 offset-md-10">
          <button class="btn btn-info w-100" id="btnEditBAST">
              Save Change
          </button>
        </div>
      </div>
...
<script type="text/javascript">    
  var Page = function () {  
    var tableInit = function(){ 
      ...
      return {
          init: function() { 
              tableInit();spkCheck();dTypeBast($('#type_bast').val());
              ...
              $(document).on('click','#btnEditBAST',function(e){
                    e.stopImmediatePropagation();
                      var evidence = [];
                      ...
                      if($('#formBast').valid()){
                            $('#pre-load-background').fadeIn();
                                        ...
                                        $.ajax({
                                                  url: base_url+'utility/edit_bast_domes',
                                                  type:'POST',
                                                  dataType : "json",
                                                  data:  formData ,
                                                  async : true, 
                                                  ...
                                                  success:function(result){
                                                    ...
                                                    setPage(base_url+"bast/view/"+result.id_bast);
